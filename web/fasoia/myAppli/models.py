...
class OffreEmploi(models.Model):
    ...
    # ===== Documents =====
    documents_exiges = models.JSONField(
        default=list,
        blank=True,
        help_text="Ex: ['CV', 'Lettre motivation', 'Diplôme', 'CNI']"
    )
...
class LettreMotivationGeneree(models.Model):
    """
    Lettre de motivation générée par IA pour un candidat
    """
    utilisateur = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
    ...
    # Offre ciblée (optionnelle — peut être général)
    ...
    titre = models.CharField(
        max_length=200,
        help_text="Ex: Lettre pour Développeur Python - Entreprise X"
    )
    contenu = models.TextField(
        help_text="Contenu de la lettre générée par IA"
    )

    # Fichiers générés
    fichier_pdf = models.FileField(
        upload_to='lettres/pdf/',
        null=True,
        blank=True
    )
    fichier_docx = models.FileField(
        upload_to='lettres/docx/',
        null=True,
        blank=True
    )
...
class DossierCandidature(models.Model):
    """
    Dossier de candidature d'un candidat pour une offre d'emploi
    """
    ...
    lettre_generee = models.TextField(
        blank=True,
        help_text="Lettre générée par IA"
    )
    lettre_generee_obj = models.ForeignKey(
        'LettreMotivationGeneree',
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='candidatures'
    ...
    @property
    def cv_final(self):
        """Retourne le CV uploadé ou le CV généré"""
        if self.cv:
            return self.cv
